                                ...
                                float p3y, float t, float rad, float *xc, float *yc, float *xb, float *yb)
{
  ...
  // now we get derivative points (use double to avoid problem on corners)
  ...
  const double l = 1.0f / sqrt(dx * dx + dy * dy);
  *xb = (*xc) + rad * dy * l;
  *yb = (*yc) - rad * dx * l;
}
                                   ...
                                   float **border, int *border_count, int source)
{
  ...
  *points_count = 0;
  ...
  for(int k = 0; k < nb; k++)
  {
    const dt_masks_point_path_t *pt = (dt_masks_point_path_t *)g_list_nth_data(form->points, k);
...
static void dt_path_get_distance(float x, float y, float as, dt_masks_form_gui_t *gui, int index,
  ...
  *inside = 1;
  ...
  if(gpt->points_count > 2 + corner_count * 3)
  {
    ...
    for(int i = corner_count * 3; i < gpt->points_count; i++)
    {
      ...
      const float dd = (gpt->points[i * 2] - x) * (gpt->points[i * 2] - x) + (yy - y) * (yy - y);
     ...
     || gui->point_border_selected >= 0)
  {
    ...
    else
    {
      ...
      if((state & GDK_SHIFT_MASK) == GDK_SHIFT_MASK)
      {
        ...
        for(int k = 0; k < nb; k++)
        {
          const dt_masks_point_path_t *point = (dt_masks_point_path_t *)g_list_nth_data(form->points, k);
