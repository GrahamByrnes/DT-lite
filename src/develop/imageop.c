...
/* default flags for modules which do not implement the flags() function */
static int default_flags(void)
{
  return 0;
}

/* default operation tags for modules which do not implement the flags() function */
static int default_operation_tags(void)
{
  return 0;
}

/* default operation tags filter for modules which do not implement the flags() function */
...
int dt_iop_load_module_so(void *m, const char *libname, const char *op)
{
  ...
  if(module->introspection_init)
  {
    if(!module->introspection_init(module, DT_INTROSPECTION_VERSION))
    {
      ...
      if(!g_module_symbol(module->module, "get_p", (gpointer) & (module->get_p)))
        goto error;
      if(!g_module_symbol(module->module, "get_f", (gpointer) & (module->get_f)))
        goto error;
...
gboolean dt_iop_so_is_hidden(dt_iop_module_so_t *module)
{
  ...
  {
    if((module->state == dt_iop_state_FAVORITE && module_view_favorite) || !module_view_favorite)
    { 
      if(!module->gui_init)
...
{
  if(pipe != dev->preview_pipe && pipe != dev->preview2_pipe)
    sched_yield();

  if(pipe != dev->preview_pipe && pipe != dev->preview2_pipe && pipe->changed == DT_DEV_PIPE_ZOOMED)
    return 1;
