...
static inline void dt_draw_histogram_8_log_base(cairo_t *cr, const uint32_t *hist, int32_t channels,
                                                ...
                                                const float *picker_values, const float hist_max)
{
  ...
  if(picker_values[0] > 0.0f)
  {
    cairo_move_to(cr, 255 * picker_values[0], 0);
    cairo_line_to(cr, 255 * picker_values[0], hist_max);
    cairo_stroke(cr);
    if(picker_values[2] - picker_values[1] > 0.0f)
    {
      cairo_rectangle(cr, 255 * picker_values[1], 0, 255 * (picker_values[2] - picker_values[1]), hist_max);
      cairo_fill(cr);
    }
  }
}
...
static inline void dt_draw_histogram_8(cairo_t *cr, const uint32_t *hist, int32_t channels, int32_t channel,
                                       const gboolean linear, const float *picker_values, const float hist_max)
{
  if(linear) // linear y
