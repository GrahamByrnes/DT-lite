...
typedef struct dt_iop_demosaic_params_t
{
  ...
  uint32_t color_smoothing; // $DESCRIPTION: "color smoothing"
...
void amaze_demosaic_RT(dt_dev_pixelpipe_iop_t *piece, const float *const in, float *out,
...
{
  int oj = 0, oi = 0;
  double sum1 = 0.0, sum2 = 0.0, gr_ratio = 1.0;

  if((FC(oj + y, oi + x, filters) & 1) != 1)
    oi++;
                                           ...
                                           const uint8_t (*const xtrans)[6], const int passes)
{
  ...
  const size_t padded_buffer_size = (size_t)TS * TS * (ndir * 4 + 3) * sizeof(float);
  char *const all_buffers = (char *)dt_alloc_align(64, dt_get_num_threads() * padded_buffer_size);
...
#pragma omp parallel for default(none) \
  ...
  {
    char *const buffer = all_buffers + dt_get_thread_num() * padded_buffer_size;
                                   ...
                                   const uint8_t (*const xtrans)[6])
{
  ...
  const size_t padded_buffer_size = (size_t)TS * TS * (ndir * 4 + 7) * sizeof(float);
  char *const all_buffers = (char *)dt_alloc_align(64, dt_get_num_threads() * padded_buffer_size);
             ...
             const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  if(qual_flags & DEMOSAIC_FULL_SCALE)
  {
    ...
    else
    {
      ...
      else if(demosaicing_method != (DT_IOP_DEMOSAIC_AMAZE || DT_IOP_DEMOSAIC_RCD))
  ...
  else
  {
    if(demosaicing_method == DT_IOP_DEMOSAIC_PASSTHROUGH_MONOCHROME)
      dt_iop_clip_and_zoom_demosaic_passthrough_monochrome_f((float *)o, pixels, &roo, &roi, roo.width, roi.width);
       ...
       dt_iop_clip_and_zoom_demosaic_passthrough_monochrome_f((float *)o, pixels, &roo, &roi, roo.width, roi.width);
    else if(piece->pipe->dsc.filters == 9u) // sample half-size raw (Bayer) or 1/3-size raw (X-Trans)
