...
int dt_history_merge_module_into_history(dt_develop_t *dev_dest, dt_develop_t *dev_src,
                                         dt_iop_module_t *mod_src, GList **_modules_used, const int append)
{
  int module_added = 1;
...
/*
    ...
    if(module_duplicate && (module_duplicate->iop_order <= 0.0 || module_duplicate->iop_order == INT_MAX))
      fprintf(stderr, "[dt_history_merge_module_into_history] invalid duplicate module module %s %s(%d)(%i)\n",
          module_duplicate->op, module_duplicate->multi_name, module_duplicate->iop_order,
          module_duplicate->multi_priority);
  ...
  else
  {
    ...
    for(GList *modules_src = dev_src->iop; modules_src; modules_src = g_list_next(modules_src))
    {
      ...
      if((_search_history_by_module(dev_src, mod_src) != NULL) // module is in history of source image
         ...
         && !dt_history_module_skip_copy(mod_src->flags()))
      {
...
static int _history_copy_and_paste_on_image_overwrite(const int32_t imgid, const int32_t dest_imgid,
                                                      GList *ops, const gboolean copy_full)
{
  int ret_val = 0;
  sqlite3_stmt *stmt;
  ...
  else // since the history and masks were deleted we can do a merge
...
{
  if(darktable.view_manager->copy_paste.copied_imageid <= 0)
    return FALSE;
